...
class Crawler {
   ...
   */
  _handlePageEvents () {
    this._page.on('error', data => void console.error(data))
    this._page.on('pageerror', text => void console.error(text))
    // this._page.on('console', data => void console.log(`${data.type()} ${data.text()} at ${this._options.url}`))
