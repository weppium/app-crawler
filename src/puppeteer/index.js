...
class PuppeteerCrawler extends EventEmitter {
   ...
   * @return {!Promise<!PuppeteerCrawler>}
   */
  static async connect (options) {
    const browser = await Puppeteer.connect(pick(options, CONNECT_OPTIONS))
    const crawler = new PuppeteerCrawler(browser, omit(options, CONNECT_OPTIONS))
    await crawler.init()
    return crawler
  }
  ...
  static async launch (options) {
    ...
    const crawler = new PuppeteerCrawler(browser, omit(options, LAUNCH_OPTIONS))
    await crawler.init()
    return crawler
  }
  ...
  constructor (browser, options) {
    ...
    this._browser.on('disconnected', () => void this.emit(PuppeteerCrawler.Events.Disconnected))
  }
  ...
  async _startRequest (options, depth, previousUrl) {
    ...
    if (skip) {
      this.emit(PuppeteerCrawler.Events.RequestSkipped, options)
      await this._markRequested(options)
      return
    }
    const allowed = await this._checkAllowedRobots(options, depth, previousUrl)
    if (!allowed) {
      this.emit(PuppeteerCrawler.Events.RequestDisallowed, options)
      await this._markRequested(options)
      return
    }
  ...
  async _request (options, depth, previousUrl, retryCount = 0) {
    this.emit(PuppeteerCrawler.Events.RequestStarted, options)
    const crawler = await this._newCrawler(options, depth, previousUrl)
    try {
      const res = await this._crawl(crawler)
      await crawler.close()
      this.emit(PuppeteerCrawler.Events.RequestFinished, options)
      ...
      if (retryCount >= options.retryCount) {
        this.emit(PuppeteerCrawler.Events.RequestFailed, error)
        await this._error(error)
        return []
      }
      this.emit(PuppeteerCrawler.Events.RequestRetried, options)
  ...
  async _getSitemapXml (sitemapUrl, options, depth, previousUrl) {
    ...
    if (!sitemapXml) {
      try {
        ...
        extend(error, { options, depth, previousUrl })
        this.emit(PuppeteerCrawler.Events.SitemapXmlRequestFailed, error)
  ...
  async _getRobot (options, depth, previousUrl) {
    ...
    if (!robotsTxt) {
      try {
        ...
        extend(error, { options, depth, previousUrl })
        this.emit(PuppeteerCrawler.Events.RobotsTxtRequestFailed, error)
        robotsTxt = EMPTY_TXT
      } finally {
  ...
  async _newCrawler (options, depth, previousUrl) {
    const page = options.page || await this._browser.newPage()
  ...
  async _followLinks (urls, options, depth) {
    if (depth >= options.maxDepth) {
      this.emit(PuppeteerCrawler.Events.MaxDepthReached)
      return
    }
    await Promise.all(map(urls, async url => {
  ...
  _checkRequestCount () {
    ...
    if (this._options.maxRequest && this._requestedCount >= this._options.maxRequest) {
      this.emit(PuppeteerCrawler.Events.MaxRequestReached)
      this.pause()
    }
  }
...
tracePublicAPI(PuppeteerCrawler)
