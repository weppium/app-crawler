...
NSCrawler.prototype.beforeExplore = function (source) {
  try {
    ...
    console.log('repeating crawling')
    this.back()
  }
}
...
NSCrawler.prototype.back = function () {
  ...
  if (this.crawlingBuffer.length === 0) {
    this.terminate('terminate due to illegal initial url')
    return
  }

  if (!this.currentNode.parent || !this.currentNode.parent.url) {
    url = this.crawlingBuffer[0].url
  } else {
    url = this.currentNode.parent.url
  }

  root.wdclient.send('/wd/hub/session/' + this.sessionId + '/url', 'post', { url: url }, null).then(() => {
    setTimeout(() => {
      this.refreshScreen()
      this.crawl()
    }, this.config.newCommandTimeout * 1000)
