...
NSCrawler.prototype.performAction = function (actions) {
  ...
  return root.wdclient
    .send('/wd/hub/session/' + this.sessionId + '/source', 'get', {}, null)
    .then(() => {
      return new Promise((resolve, reject) => {
    ...
    .then(() => {
      return root.hooks.onActionPerformed(action, this)
    })
    .then(() => {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
...
NSCrawler.prototype.refreshScreen = function (currentAction) {
  ...
  return root.wdclient.send('/wd/hub/session/' + this.sessionId + '/screenshot', 'get', null, function (data) {
    ...
    const fileDir = path.join('screenshots', fileName)

    _.mkdir(`${screenshootsDir}/screenshots`)
    fs.writeFile(path.join(screenshootsDir, fileDir), data.value, 'base64')
